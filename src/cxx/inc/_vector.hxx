...
template <class TX, class V=TX>
inline V sumValues(const TX *x, size_t N, V a=V()) {
  for (size_t i=0; i<N; ++i)
    a += x[i];
  return a;
}
template <class TX, class V=TX>
inline V sumValues(const vector<TX>& x, V a=V()) {
  return sumValues(x.data(), x.size(), a);
}
template <class TX, class V=TX>
inline V sumValues(const vector<TX>& x, size_t i, size_t N, V a=V()) {
